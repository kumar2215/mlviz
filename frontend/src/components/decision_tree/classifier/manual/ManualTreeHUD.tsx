...
const ManualTreeHUD: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const currentThreshold =
            selectedThreshold ??
            (featureStats.thresholds.length > 0
                ? featureStats.thresholds[0].threshold
                : 0);
        ...
        const colorScheme = Object.keys(
            featureStats.histogram_data.counts_by_class,
        ).map((_, i) => {
            return DEFAULT_COLORS[i % DEFAULT_COLORS.length];
        });

        console.log(
            "[ManualTreeHUD] Histogram color scheme source: DEFAULT_COLORS (Observable10)",
        );
        console.log(
            "[ManualTreeHUD] Histogram colorScheme array:",
            colorScheme,
        );
        console.log(
            "[ManualTreeHUD] Histogram class keys:",
            Object.keys(featureStats.histogram_data.counts_by_class),
        );
        console.log("[ManualTreeHUD] classNames from context:", classNames);

        renderHistogramBars(
            histGroup,
            featureStats.histogram_data,
            stackedData,
            {
        ...
        if (!exploredIndicesCache.current.has(selectedFeature)) {
            exploredIndicesCache.current.set(
                selectedFeature,
                new Set<number>(),
            );
        }
        const existingExploredIndices =
            exploredIndicesCache.current.get(selectedFeature);
        ...
        const thresholdX =
            leftMargin +
            ((currentThreshold - featureRange[0]) /
            ...
            <div className="space-y-4">
                <div className="space-y-2">
                    ...
                    <Select
                        ...
                        onValueChange={loadFeatureStats}
                    >
                ...
                {featureStats && selectedFeature ? (
                    <>
                        <div className="space-y-2">
                            ...
                            <Slider
                                ...
                                step={
                                    (featureStats.feature_range[1] -
                                        featureStats.feature_range[0]) /
                                    100
                                }
                                onValueChange={(vals: number[]) =>
                                    updateThreshold(vals[0])
                                }
                                className="py-2"
                            />
                        </div>
                    ...
                    <div className="h-[250px] flex items-center justify-center border-2 border-dashed border-slate-200 rounded-xl bg-slate-50/50">
                        <p className="text-xs text-slate-400 italic">
                            Select a feature to begin splitting
                        </p>
                    </div>
                )}
                ...
                <div className="flex flex-row gap-2 pt-2">
                    <Button
                        ...
                        disabled={
                            !selectedFeature || selectedThreshold === null
                        }
                        onClick={splitNode}
                    >
                        <Split className="w-4 h-4 shrink-0" />{" "}
                        <span className="text-xs">Split Node</span>
                    </Button>
                    <Button
                    ...
                    >
                        ...
                        <span className="text-xs">Mark as Leaf</span>
                    </Button>
                </div>
            </div>
        </CollapsibleHUD>
