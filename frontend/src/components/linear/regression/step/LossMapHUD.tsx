/**
 * Loss Map HUD
 * Renders a 2D MSE loss landscape with a live crosshair marker at the current
 * (slope, intercept). Three display modes:
 *  - "heatmap"  — coloured cells only
 *  - "contour"  — contour lines only (over a very faint fill)
 *  - "both"     — faint heatmap + contour lines on top (default)
 */
...
interface LossGrid {
    slopes: number[];
    intercepts: number[];
    /** Row-major [slopeIdx * GRID_SIZE + interceptIdx] — flat for d3.contours */
    flat: number[];
    values: number[][];
    minLoss: number;
    maxLoss: number;
...
const LossMapHUD: React.FC<LossMapHUDProps> = ({ mode: modeProp }) => {
    ...
    const [mode, setMode] = useState<LossMapMode>(modeProp ?? "both");
    ...
    const lossGrid = useMemo<LossGrid | null>(() => {
        ...
        const slopes = d3.range(GRID_SIZE).map(
            (i) => slopeMin + (i / (GRID_SIZE - 1)) * (slopeMax - slopeMin)
        );
        const intercepts = d3.range(GRID_SIZE).map(
            (i) => interceptMin + (i / (GRID_SIZE - 1)) * (interceptMax - interceptMin)
        );

        // d3.contours expects flat[col + row * width] where col=slopeIdx, row=interceptIdx
        const values: number[][] = slopes.map((m) =>
            intercepts.map((b) => computeMSE(m, b))
        );
        const flat = new Array(GRID_SIZE * GRID_SIZE);
        for (let si = 0; si < GRID_SIZE; si++) {
            for (let bi = 0; bi < GRID_SIZE; bi++) {
                flat[si + bi * GRID_SIZE] = values[si][bi];
            }
        }

        const minLoss = Math.min(...flat);
        const maxLoss = Math.max(...flat);

        return { slopes, intercepts, flat, values, minLoss, maxLoss };
    }, [visualizationData, computeMSE]);
    ...
    useEffect(() => {
        ...
        const { slopes, intercepts, flat, minLoss, maxLoss } = lossGrid;

        const xScale = d3
            .scaleLinear()
            .domain([slopes[0], slopes[slopes.length - 1]])
            .range([0, innerW]);

        const yScale = d3
            .scaleLinear()
            .domain([intercepts[0], intercepts[intercepts.length - 1]])
            .range([innerH, 0]);

        const logMin = Math.log1p(minLoss);
        ...
        if (mode === "heatmap" || mode === "both") {
            ...
            const rects: { m: number; b: number; loss: number }[] = [];
            slopes.forEach((m, si) => {
                intercepts.forEach((b, bi) => {
                    rects.push({ m, b, loss: lossGrid.values[si][bi] });
                });
            });
            ...
            g.append("g")
                ...
                .data(rects)
                .join("rect")
                .attr("x", (d) => xScale(d.m) - cellW / 2)
                .attr("y", (d) => yScale(d.b) - cellH / 2)
                .attr("width", cellW + 1)
                .attr("height", cellH + 1)
                .attr("fill", (d) => colorScale(Math.log1p(d.loss)))
        ...
        if (mode === "contour" || mode === "both") {
            ...
            const contourData = contourGen(flat);

            // d3.contours emits coordinates in grid-index space [0, GRID_SIZE).
            // Map directly to pixel space without going through domain arrays
            // (which break on fractional indices from the contour tracer).
            const projection = d3.geoTransform({
                point(px, py) {
                    this.stream.point(
                        (px / (GRID_SIZE - 1)) * innerW,
                        innerH - (py / (GRID_SIZE - 1)) * innerH,
                    );
                },
            });
            ...
            contourGroup
                ...
                .attr("fill", mode === "contour"
                    ? (d) => colorScale(Math.log1p(d.value))
                    : "none")
                .attr("fill-opacity", mode === "contour" ? 0.15 : 0)
                .attr("stroke", (d) => colorScale(Math.log1p(d.value)))
                .attr("stroke-width", 1)
    ...
    }, [lossGrid, mode, innerW, innerH, margin.left, margin.top]);
    ...
    useEffect(() => {
        ...
        const cx = xScale(currentSlope);
        const cy = yScale(currentIntercept);
        ...
        if (stepData) {
            const px = xScale(stepData.new_slope);
            const py = yScale(stepData.new_intercept);
    ...
    }, [currentSlope, currentIntercept, stepData, lossGrid, innerW, innerH]);
        ...
        >
            {!lossGrid ? (
                ...
                <>
                    ...
                    >
                        MSE = {computeMSE(currentSlope, currentIntercept).toFixed(4)}
                    </div>
                </>
            )}
        </CollapsibleHUD>
