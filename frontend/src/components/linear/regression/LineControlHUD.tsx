...
const LineControlHUD: React.FC<LineControlHUDProps> = ({
    interceptRange,
}) => {
    ...
    const iMin = interceptRange?.[0] ?? hints?.intercept_min ?? (yRange ? -ySpan * 1.5 : -500);
    const iMax = interceptRange?.[1] ?? hints?.intercept_max ?? (yRange ? ySpan * 1.5 : 500);
    const iStep = (iMax - iMin) / 1000;

    const r2 = computeR2(currentSlope, currentIntercept);

    const r2Color =
        r2 >= 0.9 ? "text-emerald-600"
        : r2 >= 0.7 ? "text-blue-600"
        : r2 >= 0.5 ? "text-amber-600"
        : "text-red-500";
    ...
    return (
        <div
            ...
            style={{ padding: fs(18), width: fs(272) }}
        >
            <h3
                className="font-bold text-slate-800 flex items-center mb-3"
                style={{ gap: fs(8), fontSize: fs(14) }}
            >
                <TrendingUp style={{ width: fs(14), height: fs(14) }} className="text-indigo-500" />
                Line Controls
            </h3>
            ...
            <div className="flex flex-col items-center mb-4" style={{ gap: fs(2) }}>
            ...
            <div className="flex flex-col" style={{ gap: fs(4), marginBottom: fs(10) }}>
                ...
                <input
                    ...
                    onChange={(e) => setCurrentLine(currentSlope, parseFloat(e.target.value))}
                    onMouseUp={() => evaluateLine(currentSlope, currentIntercept)}
                    onTouchEnd={() => evaluateLine(currentSlope, currentIntercept)}
