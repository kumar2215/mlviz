...
interface DecisionTreeStore
    extends TrainableModelState<DecisionTreeModelData>,
        PredictableModelState<DecisionTreeModelData, DTPredictionAdditionalData> {
...
export const useDecisionTree = createBaseModelStore<DecisionTreeModelData, DecisionTreeStore>(
    {
    ...
    (set, get, baseState) => {
        ...
        const modelError = null as string | null;
        const isPredicting = false;
        const predictionError = null as string | null;
        const predictionResult = null as PredictionResult<DTPredictionAdditionalData> | null;

        const getNodeByPath = (tree: TreeNode | null, path: number[]): TreeNode | null => {
            if (!tree) return null;
            let current = tree;
        ...
        const updateNodeAtPath = (
            ...
            if (index === 0) {
                const updatedLeft = tree.left
                    ? updateNodeAtPath(tree.left, restPath, updates)
                    : tree.left;
                const result = {
                    ...tree,
                    left: updatedLeft,
                ...
                const updatedRight = tree.right
                    ? updateNodeAtPath(tree.right, restPath, updates)
                    : tree.right;
                const result = {
                    ...tree,
                    right: updatedRight,
        ...
        const trainNewModel = async (
            params: Parameters = {},
        ): Promise<DecisionTreeModelData | null> => {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    throw new Error("Training failed - API returned success: false");
                }
            } catch (error) {
                console.error("Failed to train model:", error);
                set({
                    modelError:
                        error instanceof Error ? error.message : "Unknown error training model",
                });
        ...
        const clearStoredModelParams = () => {
            baseResetModelData();
            set({ modelError: null, isModelLoading: false });
        };
        ...
        const predict = async (points: Record<string, number>) => {
            ...
            if (!currentModelData?.tree) {
                set({ predictionError: "No tree available for prediction" });
                return;
            }
        ...
        const clearPrediction = () => {
            set({ predictionResult: null, predictionError: null });
        };

        const initializeManualTree = async () => {
            const { currentModelData } = get();
            const { activeDataset } = useDataset.getState();
            ...
            if (!currentModelData || treeMode !== "manual") {
                ...
                try {
                    ...
                    if (
                        ...
                        const datasetName =
                            activeDataset &&
                            typeof activeDataset === "object" &&
                            "name" in activeDataset
                    ...
                    const initialMatrix = Array(numClasses)
                        .fill(0)
        ...
        const selectManualNode = (path: number[] | null) => {
            const { currentModelData } = get();
        ...
        const loadManualFeatureStats = async (feature: string) => {
            const { currentModelData } = get();
            const { activeDataset } = useDataset.getState();
            const treeMode = currentModelData?.treeMode || null;
        ...
        const updateManualThreshold = (threshold: number) => {
            const { currentModelData } = get();
            if (!currentModelData) return;
            setCurrentModelData({
                ...currentModelData,
        ...
        const evaluateManualTree = async () => {
            const { currentModelData } = get();
            const { activeDataset } = useDataset.getState();
            const treeMode = currentModelData?.treeMode || null;
            if (!currentModelData || treeMode !== "manual") {
        ...
        const resetModelData = () => {
            console.log("[Store] Resetting model data");
        ...
        return {
            ...
            getLastParams,
            getParameters,
            // TrainableModelState (for TrainPage)
            isLoading: isModelLoading,
            error: modelError,
            data: currentModelData,
            train: trainNewModel,
            // PredictableModelState (for PredictPage)
            isPredicting,
            predictionError,
            predictionResult,
            predict,
    ...
    deriveDecisionTreeState,
);

function deriveDecisionTreeState(state: DecisionTreeStore): Partial<DecisionTreeStore> {
    const data = state.currentModelData;
    return {
        data,
        isLoading: state.isModelLoading,
        error: state.modelError,
        treeMode: data?.treeMode ?? null,
        lastTrainedParams: state.lastParams,
        manualTree: {
            ...state.manualTree,
            tree: data?.treeMode === "manual" ? data.tree : null,
            selectedNodePath: data?.selectedNodePath ?? null,
            featureStats: data?.manualFeatureStats ?? null,
            selectedFeature: data?.selectedFeature ?? null,
            selectedThreshold: data?.selectedThreshold ?? null,
        },
    };
}
