...
export const useVisualisation = create<VisualisationStore>()(
    immer((set, get) => ({
        ...
        fetchVisualisations: async (category: string) => {
            const visualisations = Object.values(get().visualisations).filter(
            ...
            try {
                const viz_files = useConfig
                    .getState()
                    .config?.categories.find((cat) => cat.config_path === category)?.files;
                ...
                const visualisations = (await Promise.all(
                    viz_files.map(async (file) => {
                        ...
                        if (!response.ok) {
                            throw new Error(
                                `Failed to fetch visualisation file: ${filePath}, status: ${response.statusText}`,
                            );
                        }
